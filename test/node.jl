@testset "constructors" begin
    ...
    @test isa(node, Node) && node.attr == "" && node.class == "" &&
        node.dataset == Array{Any, 2}(0, 0) && node.entr == 0 && node.ig == 1 &&
        node.children == []

    node2 = Node("", "", Array{Any, 2}(0, 0), 0, 1, [])
    @test isa(node2, Node) && node2.attr == "" && node.class == "" &&
        node.dataset == Array{Any, 2}(0, 0) && node2.entr == 0 &&
        node2.ig == 1 && node2.children == []

    node3 = Node("attribute")
    @test isa(node3, Node) && node3.attr == "attribute" && node.class == "" &&
        node.dataset == Array{Any, 2}(0, 0) && node3.entr == 0 &&
        node3.ig == 1 && node3.children == []

    node4 = Node("", [node3])
    @test isa(node4, Node) && node4.attr == "" && node.class == "" &&
        node.dataset == Array{Any, 2}(0, 0) && node4.entr == 0 &&
...
@testset "accessors" begin
    ...
    @test dataset(node) == node.dataset
...
@testset "show" begin
    ...
    node2 = Node("2", "", Array{Any, 2}(0, 0), 0, 1, [])
    @test sprint(show, node2) == "[2] (entr=0.0000, ig=1.0000)"

    node3 = Node("3", "", Array{Any, 2}(0, 0), 0, 1, [])
    node4 = Node("4", "", Array{Any, 2}(0, 0), 0, 1, [])
    push!(node, node2)
    push!(node2, node4)
    push!(node2, node4)
    push!(node, node3)
